...
print("=" * 70)
print("VALIDATING POLLING ON HISTORICAL ELECTIONS (FIXED)")
print("=" * 70)
...
# Process historical polls (use full state names!)
...
# Process 2020/2024 polls (use full state names!)
...
# Create polling features using FULL STATE NAMES
def get_polling_features(state_full, year):
    """Extract polling features using full state name"""
...
# Check coverage
polling_coverage = polling_df.groupby('election_year').agg({
    'num_polls': ['sum', 'mean'],
    'poll_margin': lambda x: x.notna().sum()
})
...
polling_features_list = ['final_poll_dem', 'final_poll_rep', 'poll_margin']
...
for test_year in test_years:
    ...
    print(f"\nStates with polling: {len(test_with_polls)}/51")
    ...
    print(f"\nECONOMICS ONLY:")
    print(f"  Overall: {acc_econ:.1%} ({int(acc_econ*51)}/51)")
    if swing_acc_econ is not None:
        print(f"  Swing states: {swing_acc_econ:.1%}")
    
    print(f"\nECONOMICS + POLLING:")
    print(f"  Overall: {acc_all:.1%} ({int(acc_all*51)}/51)")
    if swing_acc_all is not None:
        print(f"  Swing states: {swing_acc_all:.1%}")
    
    improvement = acc_all - acc_econ
    swing_improvement = (swing_acc_all - swing_acc_econ) if (swing_acc_all and swing_acc_econ) else 0
    print(f"\n{'🎯' if improvement > 0 else '⚠️'} IMPROVEMENT: {improvement:+.1%} overall, {swing_improvement:+.1%} swing states")
    ...
    for state in swing_states:
        if state in test_data_reset['state'].values:
            ...
            actual = 'R' if y_test.iloc[idx] == 1 else 'D'
            econ_pred = 'R' if pred_econ[idx] == 1 else 'D'
            all_pred = 'R' if pred_all[idx] == 1 else 'D'
            
            econ_check = '✓' if pred_econ[idx] == y_test.iloc[idx] else '✗'
            all_check = '✓' if pred_all[idx] == y_test.iloc[idx] else '✗'
            
            row = test_data_reset.iloc[idx]
            polls = int(row['num_polls'])
            margin = row['poll_margin'] if pd.notna(row['poll_margin']) else 0
            
            print(f"  {state}: Actual {actual} | Econ: {econ_pred} {econ_check} | +Poll: {all_pred} {all_check} | Polls: {polls:3d} (margin: {margin:+.1f}%)")
...
for _, row in results_df.iterrows():
    ...
    if pd.notna(row['swing_econ']):
        print(f"  Swing improvement:  {row['swing_improvement']:+.1%}")

avg_improvement = results_df['improvement'].mean()
avg_swing_improvement = results_df['swing_improvement'].mean()

print(f"\nAverage improvement: {avg_improvement:+.1%} overall")
print(f"Average swing improvement: {avg_swing_improvement:+.1%}")
...
if avg_improvement >= 0.05:
    ...
    print("\n⚠️ NO BENEFIT: Polling doesn't help")
